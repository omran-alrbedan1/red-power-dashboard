...
> **Maintenance media-contract + review fixes (2026-09-12):** Aligned the maintenance API layer with the backend after review (Priorities 1–5). Media uploads no longer map to the card detail: `uploadPhotos` now returns `MaintenancePhoto[]` and `uploadSignature` returns `MaintenanceSignature` (`contentUrl` metadata), dropping the incorrect `.then(toDetail)`; download URLs now hit the `/photos/:id/content` and `/signature/content` binary endpoints. Upload mutations invalidate the card via `variables.cardId` instead of `data.id`. Added `maintenanceApi.update(cardId, UpdateMaintenanceCardInput)` (`PATCH /maintenance-cards/:id`, editing subset of create fields, never `customerId`/`vehicleOwnershipId`/`receivedAt`/`requiredWorks`). Extracted `toApiWorkStatus` mapper for work-status updates. `useMaintenanceCard` now validates the route ID (`Number.isInteger(id) && id > 0`, mirroring customers). Options queries bumped to 5m stale / 30m gc (near-static config). Details page disables Close when required work remains (backend remains the authority). Removed the unreferenced placeholder `MaintenancePage.tsx`; `/maintenance` routes to the full `MaintenanceListPage` already. The previously claimed `maintenanceApi.dashboardStats()`/`customerHistory()` were already gone. tsc, lint, and build pass.

> **Maintenance dead-code sweep (2026-09-12):** Maintenance is now fully API-backed and matches the customers feature conventions. Removed the legacy in-memory mock `maintenance.service`, its `maintenance.data` fixtures, the superseded `maintenance-keys.ts`, and the zero-reference mock-era hooks (`useActivityTimeline`, `useUpdateCardStatus`, `useHistorySource`, `useCardDetailForHistory`, `usePhotoUpload`, `useSignatureUpload`, `useCreateWorkItem`, `useDeleteWorkItem`, `useUpdateWorkItem`, `useUpdateMaintenanceCard`, `useMediaDownload`), components (`WorkItemEdit`, `CustomerVehicleSelector`), and mock types (`maintenance.types`, `work-item.types`, `activity.types`, `inspection.types`, `approval.types`, `summary.types`). Both former mock consumers (dashboard stats, customer history) were already migrated to `/dashboard/stats` and `/customers/:id/maintenance-history`. Dropped the deprecated `maintenanceApi.customerHistory`/`toSummary` and the `customerHistory` query key. `useMaintenanceOptions` and `useSelectorData` now use `maintenanceQueryKeys`; used queries gained `STALE_TIME`/`GC_TIME` and used mutations gained i18n sonner toasts (`messages.*`/`errors.*` keys in `ar`/`en`). tsc, lint, and build pass.

> **Receipt wizard error + cleanup (2026-09-12):** Fixed the `/maintenance/new` drop-down bug: `MultiSelectField`'s trigger `<Button>` had no `type`, so inside the `<form>` it defaulted to `submit` — opening the "condition at receipt" multi-select submitted the card (triggering full-form validation errors and the create-card toast). The trigger is now `type="button"` with `role="combobox"`/`aria-expanded`, outside-click/Escape close, cmdk `value={option.label}` for reliable search, and optional `searchPlaceholder`/`emptyMessage` props (wired to `common.form.*`, Arabic-first) forwarded through `CustomFormField`. Save no longer validates hidden steps: `handleSave` triggers only step-3 fields and parses values (`zodResolver` output) instead of `form.handleSubmit` validating all steps, so existing-customer/vehicle selection no longer fails silently. Picking an existing customer now auto-fills name/phone/email (vehicle fields already auto-filled). Options/selector queries surface explicit loading skeletons, inline error+retry (`options.error`, `customer/vehicle.selectError/loading/empty`), and `useMaintenanceOptions` retries once. Replaced the raw `["customers"]` invalidation with `customerQueryKeys.all`, moved the fuel map into a `toApiFuelLevel` domain helper, and extracted the wizard header (`WizardSteps`) and the post-create media upload step (`MediaReceiptStep`). The duplicate `condition.options` label on the items-left multi-select now uses `itemsLeft.title`. tsc (4 pre-existing errors only), lint, and build pass.

> **Ownership guard + picker submit fix (2026-09-12):** The recurring backend 409 "سجل الملكية المحدد لا يخص العميل" came from two compounding issues on `/maintenance/new`. (1) `TimePickerField` and `ComboboxField` triggers also omitted `type`, so opening the delivery-time picker submitted the card; both now use `type="button"`, add `role="combobox"`/`aria-expanded`, outside-click/Escape close, and localized placeholders (`common.datePicker.*`/`common.timePicker.*`). (2) The wizard allowed combining any customer with any existing vehicle; the backend requires the card's customer to own the vehicle, so "new customer + vehicle registered to another customer" always 409'd. `VehicleSelect` now exposes the vehicle's owner (`ownerCustomerId`/`ownerCustomer`) from the selector data, and the page tracks `selectedVehicleId` (display) separately from `vehicleOwnershipId` (payload, fixing the trigger display mismatch). Picking a vehicle whose owner differs from the selected customer — or when no customer is selected — no longer binds it: the page shows an inline conflict (`vehicle.ownershipMismatch`, brand-red) with "use vehicle owner" (`adoptVehicleOwner` adopts the owner as the customer + autofills + binds) and cancel actions, so an invalid `customerId`/`vehicleOwnershipId` pair can never reach the backend. tsc (4 pre-existing errors only), lint, and build pass.

> **Card edit/update (2026-09-12):** `/maintenance/:id` is no longer read-only. Added a `useUpdateMaintenanceCard` hook (PATCH `/maintenance-cards/:id`, invalidates detail/list/timeline/dashboard stats, toasts `messages.cardUpdated`/`errors.updateFailed`) and an `EditReceiptCard` section on the details page. Editing toggles via "Edit Card" in the header card (hidden for closed cards) and is only enabled while the card is open. It reuses `VisitSection` + `DeliverySection` + a mileage input hydration from the loaded detail, validates with a new `editReceiptFormSchema`, and sends only the PATCH contract fields (`UpdateMaintenanceCardInput`): mileage, fuelLevel, customerComplaint, inspectionNotes, customerApproved (+approvalName, and customerApprovedAt only on a fresh approve transition), expectedDeliveryAt, visitReasonIds, vehicleConditionOptionIds, vehicleItemOptionIds. Customer/vehicle identity is intentionally immutable on an existing card. While editing, the read-only reason/condition/itemsLeft/work/approval cards are hidden and restored on save/cancel. `buildExpectedDelivery` was extracted from the create page into `utils/expected-delivery.ts` (shared). tsc (4 pre-existing errors only), lint, and build pass.

> **Maintenance RTL/LTR cleanup (2026-09-12):** Corrected direction handling across the maintenance feature. The app root `dir` already follows the active language (`i18n/config.ts` sets `documentElement.dir`), so hardcoded `dir="rtl"` on translated-content fields broke English mode, and hardcoded `dir="ltr"` on `ar-SA`-formatted dates broke Arabic mode. Translated-content fields (names, make/model, transmission/fuel selects, textareas, switches, approval name, date/time pickers) now use a language-aware `dir={isAr ? "rtl" : "ltr"}` via a `formDir` const in each section (`CustomerFields`, `VehicleFields`, `VisitSection`, `WorkSection`, `DeliverySection`, `CustomerVehicleSection`). Inherently-LTR content keeps `dir="ltr"` (phone, email, plate#, VIN, year, mileage, cost, card numbers). List dates switched to `dir="auto"` so `ar-SA` dates render RTL and `en-GB` render LTR (list table + mobile cards). Removed the no-op `ltr` class from detail-page spans (only `[dir="ltr"]` exists in CSS, not `.ltr`). tsc (4 pre-existing errors only), lint, and build pass.

> **Maintenance table extraction (2026-09-12):** Extracted a reusable `MaintenanceTable` (`components/MaintenanceTable.tsx`) mirroring the customers' `CustomerTable` pattern: it wraps the shared `DataTable` in the rounded card container, owns columns (card number LTR, customer, vehicle make/model+plate, status badge, receipt date) and the `MaintenanceMobileCard`, and receives the empty state via the `emptyState`/`emptyMessage` props. `MaintenanceListPage` now just passes data, row click, pagination, and the empty-state config (icon/title/description/primary action) through — the empty state is rendered INSIDE the `DataTable` (desktop: `colSpan` row; mobile: inline), matching the customers feature, instead of replacing the table. The status column and the mobile card use the shared `StatusBadge` from `components/shared/badges` (its `"open"|"closed"` union matches the card status) with the translated maintenance label (`statuses.*`); the maintenance-specific `StatusBadge.tsx` remains only for the receipt header. tsc (4 pre-existing errors only), lint, and build pass.

> **Maintenance type hardening (2026-09-13):** Removed fabricated values so the maintenance domain never invents zeros or plausible-looking defaults. `maintenance.mapper` no longer synthesizes `{ id: 0, name: "", phone: "" }` customers or `{ id: 0, ... }` vehicles: `MaintenanceCardListRow` and `MaintenanceCardDetail` now surface `customer`/`vehicle` as nullable (and `vehicleOwnershipId` as `number | null`), and all consumers (list table, mobile cards, dashboard recent-cards, receipt details, edit approval-name fallback) render `"-"` fallbacks. Fuel level is single-sourced through `PersistedFuelLevel = Lowercase<ApiFuelLevel>` with exhaustive `mapFuelLevel` (API→domain) and `toApiFuelLevel` (domain→API) mappers, replacing the silent `"HALF"` fallback for unknown values; both receipt schemas constrain `fuelLevel` via `z.enum`. `MaintenanceFilterValues.status` is now `MaintenanceCardStatus | ""` so the list page needs no `as` cast, and the `receivedAt` range end uses `addDays(range.to, 1)` instead of the raw `86400000` magic number. tsc (4 pre-existing errors only), lint, and build pass.
